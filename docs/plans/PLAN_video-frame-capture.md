...
- (후속 ✅ 정지 영상 스킵) 팟캐스트형(한 이미지 고정) 영상은 프레임 캡처를 건너뜀. `is_static_video`가 다운로드 후 영상 중간 70% 구간 5지점을 다운스케일 추출(`_extract_small_frame` 128px)해 기준 프레임 대비 ffmpeg `blend=difference,signalstats` YAVG(평균 휘도차, 0-255)를 측정 → `decide_static`이 최대 diff < `STATIC_DIFF_THRESHOLD`(4.0)면 정지로 판정하고 `{}` 반환(이미지 0장). **실측 캘리브레이션**: 동일프레임 0.0 / NPR 팟캐스트(Wait Wait·It's Been a Minute) 0.0→스킵 / Vox·johnnyharris 설명영상 82~90→정상 캡처. 정지(0)와 움직임(82+) 간격이 압도적이라 임계값 4.0 안전. 순수함수 `static_sample_points`·`decide_static` 단위테스트 9개(총 119 통과).
- (FRAMES_PER_VIDEO·한글) 사용자 요청 반영: 캡처 6장(`.env` FRAMES_PER_VIDEO=6, 코드 기본 5), 한글 섹션은 이미지 미삽입(영문 기사에만 frame 부착).
