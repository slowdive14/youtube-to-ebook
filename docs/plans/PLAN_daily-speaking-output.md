...
- (후속 ✅ 워밍업 문장 실용화) 드릴이 기사 verbatim B2+ 인용이라 너무 어려움 → `generate_speaking_prompt`에 **`shadow`**(구체·회화체 B1-B2 실용문 3개, verbatim 금지) 추가. main.py에서 옛 `generate_drill_sentences` 호출 은퇴(신규 이슈 드릴/`/drill` 미생성). 라이브: "Can I trust you with this?" 등 실용문 산출.
- (후속 ✅ 공통 패턴 관통) 사용자 피드백: 1↔2단계가 서로 무관하고 마지막 응용도 따로 놂. → `speakingPrompt`에 **`patterns`**(재사용 패턴 1~3개) 추가. 각 패턴 = `pattern`(___ 슬롯) + `s1`(그 패턴 쓴 모델문장, 따라말하기) + `s2`(같은 패턴 다른 문장 + `answer`=빈칸어). `/speak` 워밍업이 **한 패턴을 관통**: 1단계 s1 따라 → 2단계 s2 빈칸(같은 패턴이라 1↔2 연결) → 3단계 그 패턴으로 **내 상황** 말하기(신규 `mode:'apply'`). 라이브: "It's hard to ___" → s1 "trust someone you just met" / s2 "focus when you're tired" 식으로 연결 확인. 옛 shadow/drill은 maxStep=1 단일단계로 폴백. 생성 로직 재설계(prompt/parse/스키마/export/page/API). 152 테스트 통과.
- (후속 ✅ 빈칸·번역 드릴 살리기 = 3단계 사다리) 사용자 요청: 어려운 드릴을 우리 엔진의 연장선에서 활용. → 워밍업 한 문장을 **shadow→fill→translate** 로 올라가게: ①따라 말하기 ②핵심어 빈칸(전체 문장 말하기, shadow 모드 재사용) ③한국어만 보고 영어로(신규 **`mode:'translate'`**, 의미 전달이면 패러프레이즈도 인정). 문장 1세트(`shadow`의 en/ko)를 3가지로 재활용 — 새 생성 0. translate REST 실측: 패러프레이즈 인정 + 자연스러운 corrected 제시. 옛 4단계 실패원인 3가지(Web Speech·정답일치·verbatim)가 모두 해소된 위에서 빈칸/번역이 의미 있는 중간 난도 계단으로 부활.
