...
[[SUM]] LLMs, being autoregressive and prone to hallucination (e.g., 20% of the time), are unsuitable for mission-critical applications like autonomous vehicles or chip design, as their "guessing game" based on probabilities is costly and lacks internal transparency.
...
[[SUM]] Unlike LLMs with "tunnel vision," Energy-Based Models (EBMs) offer a "bird's-eye view," allowing them to foresee and adapt to all possible scenarios due to their non-autoregressive, token-free, and inspectable architecture, preventing wasted compute and enabling internal verification.
...
[[SUM]] EBMs operate on the physics principle of energy minimization, constructing an "energy landscape" where the lowest points represent the most probable and optimal states, directly mapping data without tokens to find the "most comfortable configuration."
...
[[SUM]] While LLMs excel at pattern recognition, EBMs with latent variables go further by "understanding" data, inferring basic knowledge and rules about the world, making them adept at working directly with numerical data and sparse information without language translation.
...
[[SUM]] EBMs can generate formally verified code and mathematical proofs, ensuring logical compatibility and compliance by being constrained to follow precise human-defined rules, which is crucial for safety-critical applications like autopilot systems where LLMs can misbehave.
...
[[SUM]] Despite LLM's current dominance, Logical Intelligence views EBMs as complementary, handling spatial reasoning, engineering, and data analysis tasks requiring precision, making LLM deployments cheaper and more reliable by outsourcing suitable tasks to EBMs.
...
[[SUM]] The job of "driver" has persisted for centuries, unlike knocker-uppers or lamplighters, but human driving is inherently flawed due to distraction and fatigue, causing 1 in 100 deaths, making driverless cars a safer alternative already providing millions of rides.
...
[[SUM]] The DARPA Grand Challenge in 2004, though initially a "hysterical disaster," jump-started driverless car development, leading to Sebastian Thrun's software-first approach with "Stanley," which won the 2005 challenge and proved the technology's potential.
...
[[SUM]] Larry Page hired Thrun to lead Google's secret Project Chauffeur in 2009, aiming to log 100,000 miles and complete ten tricky 100-mile routes, which they achieved in just over a year by teaching cars subtle human driving behaviors like "nudging" to ensure passenger comfort.
...
[[SUM]] After completing the Larry 1K, Google debated whether to build an assistive technology or disruptive robo-taxis, with Thrun favoring the latter, as competition emerged and the team faced internal schisms, marking a new phase for the driverless car revolution.
...
[[SUM]] Nvidia's core identity is transforming electrons into valuable tokens, an "incredible journey" of "artistry, engineering, science, and invention" that is inherently difficult to commoditize, embedding Nvidia in the five-layer AI ecosystem.
...
[[SUM]] AI will not commoditize enterprise software; instead, the number of AI agents and tool users will grow exponentially, leading to a "skyrocket" in software company revenues as agents become proficient with tools.
...
[[SUM]] Nvidia's supply chain moat is built on "enormous commitments" and Huang's ability to inspire and align upstream partners, convincing them to make massive investments by demonstrating Nvidia's capacity to buy and sell their supply.
...
[[SUM]] Hardware bottlenecks are temporary, solvable within "two or three years" with demand signals and investment, but the real long-term concern is fundamental energy policy to support "AI factories" and reindustrialization.
...
[[SUM]] Nvidia focuses on "accelerated computing," not just tensor processing, enabling 10x to 100x algorithmic leaps through programmable architectures like CUDA and "extreme co-design" for new models like MoEs.
...
[[SUM]] CUDA endures due to its rich ecosystem, a "several hundred million GPUs" install base ensuring software runs "everywhere," and offering the "best performance per TCO in the world," creating a powerful flywheel effect.
...
[[SUM]] Huang admitted a past "mistake" in not realizing AI labs like Anthropic needed huge upfront investments from suppliers, but Nvidia is now investing in key AI labs and "neoclouds" to nurture the ecosystem without becoming a cloud provider.
...
[[SUM]] Nvidia allocates GPUs based on forecasting, purchase orders, and "first in, first out," not by favoring the "highest bidder," as it's a "bad business practice" to deviate from set prices.
...
[[SUM]] Nvidia's dominance stems from strategic vision, technological innovation, ecosystem cultivation, and market understanding, reinforced by programmability, a vast install base, and a dynamic investment strategy, making its position indispensable in AI.
...
[[SUM]] The US imposed a naval blockade after failed negotiations, finding Iran's control and "tolls" in the Strait of Hormuz intolerable, aiming to reverse Iranian influence.
...
[[SUM]] A naval blockade, militarily defined as an act of war, involves 10,000 US sailors and a dozen warships intercepting vessels to cut off Iran's oil exports and force negotiations on nuclear issues.
...
[[SUM]] High risks include Iranian retaliation against US ships, China's wrath over deprived oil, and Iran restarting attacks on regional energy infrastructure, which could take years to restore.
...
[[SUM]] The US Navy's "quarantine" outside the strait uses drones and radio contact to deter ships, with six vessels reportedly turning around, partially stopping Iranian oil shipments.
...
[[SUM]] Shipping insurance costs have soared, and experts believe the strait will not return to pre-war free passage, necessitating alternative routes, diversified oil sources, or an accelerated energy transition.
...
[[SUM]] The blockade is a test of wills, with Iran betting Trump will back down due to rising gas prices, while the US hopes economic pressure will force Tehran to accept American terms.
...
[[SUM]] Sustaining the blockade drains US resources from other critical areas like the Indo-Pacific and Europe, requiring 10,000-50,000 personnel, as France and Britain develop their own plan for the strait.
...
[[SUM]] LLM은 환각을 일으킬 위험이 있어 자율주행차나 비행기 조종 등 미션 크리티컬 시스템에 적용하기 어렵고, 내부 검증이 불가능한 '블랙박스'라는 한계가 있습니다.
...
[[SUM]] EBM은 물리학의 에너지 최소화 원리를 적용하여 가장 안정적인 상태를 찾아가는 '토큰 프리' 모델로, 비순차적이고 내부 검증이 가능하며 자기 정렬 능력을 갖춰 LLM보다 효율적입니다.
...
[[SUM]] LLM이 패턴을 인식하는 반면, EBM은 '잠재 변수'를 통해 데이터의 근본적인 규칙과 지식을 '이해'하며, 이는 상징 AI의 한계를 극복하고 새로운 상황에서도 추론 및 계획을 가능하게 합니다.
...
[[SUM]] LLM은 '터널 시야'로 한 번에 한 방향만 선택해 오류에 취약하지만, EBM은 '조감도'로 모든 경로를 파악하여 효율적이고 정확하게 문제를 해결하며, LLM 코드의 일관성 부족 문제도 보완합니다.
...
[[SUM]] 로지컬 인텔리전스는 EBM을 활용해 '형식적으로 검증된 코드'를 생성하고 코딩을 자동화하며, 인간이 설정한 '제약 조건'을 항상 준수하여 자율주행차 등 예측 불가능한 행동이 치명적인 분야에 필수적입니다.
...
[[SUM]] LLM은 정체기에 도달하고 있지만, EBM은 LLM 스택의 하위 레이어로 통합되어 세금 계산이나 공간 추론 같은 특정 작업을 위임받아 LLM의 비용을 절감하고 효율성을 높일 수 있습니다.
...
[[SUM]] LLM의 한계를 보완하는 EBM은 에너지 최소화 원리로 데이터의 규칙을 이해하고 예측 가능하게 작동하며, LLM과 EBM의 하이브리드 접근 방식이 안전하고 신뢰할 수 있는 AI 시대를 열 것입니다.
...
[[SUM]] 200년 전 사라진 직업들과 달리 '운전자'는 남아있지만, 미래에는 기계를 지칭하게 될 것이며, 웨이모 같은 로보택시가 이미 미국 10개 도시에서 수백만 건 운행하며 인간 운전의 위험을 대체하고 있다. 초기 자동차 도입 시 '붉은 깃발 법' 같은 저항이 있었으나, 결국 법률, 면허, 도로 개선 등으로 사회가 적응했다.
...
[[SUM]] 젠슨 황 CEO는 엔비디아가 '전자(electrons)를 토큰(tokens)으로 변환하는 예술'을 통해 AI 시대의 핵심 동력으로 자리매김하며, 이 과정은 상품화하기 어렵고 엔비디아가 핵심 변환 작업을 주도한다고 강조합니다.
...
[[SUM]] 황 CEO는 AI로 인해 대부분의 소프트웨어 기업이 '도구 제작자'가 될 것이며, AI 에이전트의 증가로 엑셀, 파워포인트 같은 도구 사용이 폭발적으로 늘어 소프트웨어 기업 성장을 이끌 것이라고 전망합니다.
...
[[SUM]] 엔비디아는 약 1천억 달러의 구매 약정으로 희소 부품을 선점하고, 공급망 CEO들을 설득하여 투자를 유도하며, CoWoS 패키징 같은 병목 현상을 선제적으로 해결하지만, 장기적으로는 숙련공 부족과 에너지 정책이 더 큰 병목이라고 지적합니다.
...
[[SUM]] 황 CEO는 엔비디아가 TPU가 아닌 분자 역학, 유체 역학 등 광범위한 분야에 사용되는 '가속 컴퓨팅'을 구축했으며, CUDA를 통한 '일반적인 프로그래밍 가능성'과 '극단적 공동 설계'가 AI 알고리즘 발전을 촉진하는 핵심 강점이라고 설명합니다.
...
[[SUM]] 엔비디아는 하이퍼스케일러에 최고의 성능 대비 총소유비용을 제공하며, 과거의 실수를 인정하고 오픈AI, 앤트로픽 등 유망 AI 랩에 적극 투자하며, 코어위브 같은 신생 AI 클라우드를 지원하여 생태계 번성을 돕습니다.
...
[[SUM]] 황 CEO는 GPU 할당이 '최고가 입찰자' 방식이 아닌 수요 예측, 구매 주문서, '선착순' 원칙을 따른다고 부인하며, 엔비디아는 가격을 설정하고 고객이 구매 여부를 결정하는 방식으로 사업을 운영한다고 강조합니다.
...
[[SUM]] 엔비디아의 해자는 '전자에서 토큰으로'의 변환이라는 기술적 리더십, 유연한 CUDA 생태계, 공급망 영향력, 그리고 '필요한 만큼, 최소한으로'라는 철학 아래 파트너십을 통해 AI 산업 혁신을 촉진하는 다층적인 요소들로 이루어져 있습니다.
...
[[SUM]] 이란이 호르무즈 해협 통행료를 부과하자 미국은 10,000명 이상의 병력과 12척 이상의 전함을 배치, '검역' 형태로 이란의 석유 수출을 막아 협상 테이블로 끌어내려 합니다.
...
[[SUM]] 이란의 보복, 이란 석유의 90%를 수입하는 중국의 반발, 그리고 페르시아만 에너지 인프라 공격으로 인한 글로벌 에너지 시스템 혼란이 주요 위험입니다.
...
[[SUM]] 초기 48시간 동안 이란 항구를 떠났던 6척의 선박이 회항하며 이란 석유 수출 차단에는 성공했으나, 아랍 국가 선박의 안전한 해협 통과 보장은 미완의 과제로 남아있습니다.
...
[[SUM]] 선사들의 불안감이 높고, 이란이 해협 통제의 힘을 인지하면서 호르무즈 해협은 더 이상 자유로운 국제 수로가 되기 어려울 것이며, 국제 컨소시엄 구성 등 대안이 논의됩니다.
...
[[SUM]] 걸프만 국가들의 대안 운송 경로 개발, 다른 지역 석유 수요 증가, 대체 에너지 부상이 예상되며, 미국과 이란 중 누가 먼저 봉쇄의 고통을 견딜지 '고통 감내' 게임이 진행됩니다.
...
[[SUM]] 미 국방부는 봉쇄 지속 가능성을 밝혔으나, 다른 지역 군사 임무에 막대한 비용을 치러야 하며, 트럼프 대통령의 전면전 재개는 지지층 분열 등으로 어려울 것으로 보입니다.
